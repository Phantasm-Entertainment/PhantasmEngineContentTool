...
namespace PECT
{
    struct FontCharData
    {
        ...
        inline FontCharData(std::uint8_t c, std::int32_t bx, std::int32_t by, std::int32_t a, std::uint16_t w, std::uint16_t h, std::shared_ptr<std::uint8_t[]> d) : Code(c), BearingX(bx), BearingY(by), Advance(a), Width(w), Height(h), Data(d) { }
